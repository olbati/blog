...
class business_Contact_Info extends WP_Widget {



// constructor

    function business_contact_info() { 
	...
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'business'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'address' ); ?>"><?php _e( 'Enter your address', 'business' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'phone' ); ?>"><?php _e( 'Enter your phone number', 'business' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php _e( 'Enter your email address', 'business' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['business_contact_info']) )

			delete_option('business_contact_info');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('business_contact_info', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'business_contact_info', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Contact info', 'business' );
		...
		if( ($address) ) {
			...
			echo '<span>' . __('Address: ', 'business') . '</span>' . $address;
		...
		if( ($phone) ) {
			...
			echo '<span>' . __('Phone: ', 'business') . '</span>' . $phone;
		...
		if( ($email) ) {
			...
			echo '<span>' . __('Email: ', 'business') . '</span>' . '<a href="mailto:' . $email . '">' . $email . '</a>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'business_contact_info', $cache, 'widget' );
